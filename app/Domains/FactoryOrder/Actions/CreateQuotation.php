...
class CreateQuotation
{
    public function execute(QuotationCreateData $data)
    {
        $quotation = Quotation::create([
            ...
            'sales_agent_id' => $data->salesAgentI,
        ...
        $itemsGrossTotal = 0;
        collect($data->items)->each(function (QuotationItemData $itemData) use (&$itemsGrossTotal, $quotation) {
            ...
            collect($itemData->embellishments)->each(function (QuotationCreateEmbellishmentData $embellishmentData) use (&$embellishmentsTotal, $item) {
